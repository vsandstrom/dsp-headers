  ...
  class ADSREnvelope {
    ...
    inline float next_value(
      ...
      if (count >= static_cast<size_t>(dur * self.samplerate)) {
        switch (stage) {
          case EnvStage::ATK: { stage = EnvStage::DEC; break; }
          case EnvStage::DEC: { stage = EnvStage::SUS; break; }
          case EnvStage::REL: { stage = EnvStage::ATK; self.playing = false; break; }
          default: { break; }
        }
        self.count = 0;
        self.prev = env;
